    ...
    <div className="flex flex-col h-full animate-in slide-in-from-right duration-500">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <button 
            onClick={onClose}
            className="p-2 rounded-lg bg-[var(--bg-input)] text-[var(--text-dimmed)] hover:text-red-600 hover:bg-[var(--bg-primary)] transition-all border border-[var(--border-subtle)]"
          >
          ...
          <div>
            ...
            <p className="text-[var(--text-dimmed)] text-[10px] font-bold uppercase tracking-widest mt-0.5">Moteur Gemini v1.5 Pro</p>
          </div>
        </div>

        <div className="flex gap-4">
           <div className="flex flex-col items-end">
              <span className="text-[9px] font-black uppercase tracking-widest text-[var(--text-dimmed)]">Quota Articles</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-grow">
        ...
        {/* Radar à Sujets */}
        <div className="lg:col-span-2 glass-card border-red-600/20 bg-[var(--bg-secondary)] flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <h4 className="flex items-center gap-3 text-sm font-black uppercase tracking-widest text-[var(--text-main)]">
              <Sparkles className="w-4 h-4 text-red-600" /> Radar à Opportunités SEO
            </h4>
            <button 
              onClick={loadRadar}
              disabled={loadingRadar}
              className="p-2 rounded-lg bg-[var(--bg-input)] text-[var(--text-dimmed)] hover:text-red-600 transition-all border border-[var(--border-subtle)] disabled:opacity-50"
            >
          ...
          <div className="space-y-4 flex-grow overflow-y-auto pr-2">
            {loadingRadar ? (
              Array(3).fill(0).map((_, i) => (
                <div key={i} className="h-32 bg-[var(--bg-input)] rounded-2xl animate-pulse" />
              ))
            ) : (
              topics.map((topic, i) => (
                <div 
                  key={i} 
                  className={`group glass-card bg-[var(--bg-input)] border-[var(--border-subtle)] hover:border-red-600/30 p-5 cursor-pointer transition-all ${generatingArticle === topic.title ? 'opacity-50' : ''}`}
                ...
                >
                  ...
                  <p className="text-[var(--text-dimmed)] text-[11px] leading-relaxed line-clamp-2">{topic.description}</p>
        ...
        {/* Status / Preview */}
        <div className="glass-card flex flex-col justify-center items-center text-center p-8 bg-[var(--bg-secondary)] border-[var(--border-subtle)]">
           {generatingArticle ? (
             <>
               ...
               <h4 className="text-xl font-black uppercase tracking-tighter mb-4 animate-pulse text-[var(--text-main)]">Rédaction en cours...</h4>
               <p className="text-[var(--text-dimmed)] text-xs italic max-w-xs uppercase font-medium leading-relaxed">
                  Gemini analyse le sujet : <br />
                  <span className="text-[var(--text-main)] not-italic mt-2 block">"{generatingArticle}"</span>
               </p>
               <div className="mt-12 space-y-3 w-full">
                  ...
                  <div className="flex items-center gap-3 text-[10px] font-black text-[var(--text-dimmed)] uppercase">
             ...
             <>
               <div className="w-16 h-16 bg-[var(--bg-input)] rounded-2xl flex items-center justify-center mb-6 border border-[var(--border-subtle)]">
                  <Sparkles className="w-8 h-8 text-[var(--text-dimmed)]" />
               </div>
               <h4 className="text-lg font-black uppercase tracking-tighter mb-4">Prêt à Générer ?</h4>
               <p className="text-[var(--text-dimmed)] text-[11px] uppercase tracking-widest leading-relaxed">
