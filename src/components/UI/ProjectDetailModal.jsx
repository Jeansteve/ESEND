  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        className="relative w-full max-w-6xl max-h-[95vh] bg-slate-900 rounded-[2rem] md:rounded-[3rem] border border-white/10 overflow-hidden shadow-3xl flex flex-col"
      >
        <button 
          onClick={onClose} 
          className="absolute top-4 right-4 md:top-6 md:right-6 z-50 p-2 md:p-3 rounded-full bg-black/40 text-white hover:bg-red-600 hover:scale-110 transition-all border border-white/10"
        >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar text-left">
          ...
          <div className="relative aspect-[21/9] min-h-[300px] w-full overflow-hidden">
             ...
             <div className="absolute bottom-6 left-6 md:bottom-10 md:left-16 right-6 md:right-16">
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  <span className="px-3 py-1 md:px-4 md:py-1 rounded-full bg-red-600 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-white">{project.tag}</span>
                  <span className="flex items-center gap-1.5 text-slate-300 text-[9px] md:text-[10px] font-bold uppercase tracking-widest"><MapPin className="w-3 h-3 text-red-500" /> {project.location}</span>
                </div>
                <h3 className="text-2xl md:text-5xl lg:text-6xl font-black uppercase text-white leading-tight">{project.title}</h3>
             </div>
          </div>
          ...
          <div className="px-6 md:px-16 py-8 md:py-12 grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
             ...
             <div className="lg:col-span-8 space-y-8 md:space-y-12">
                ...
                <div className="article-preview-content prose prose-invert prose-red max-w-none 
                      ...
                      return (
                        <div className="pt-12 border-t border-white/5">
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              {gallery.map((url, i) => (
                                 <div key={i} className="aspect-square rounded-2xl md:rounded-3xl overflow-hidden border border-white/5 hover:border-red-600/30 transition-all group">
             ...
             <div className="lg:col-span-4 space-y-8">
                <div className="p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] bg-slate-800/50 border border-white/10 space-y-8 sticky top-8">
                   <div>
          ...
          {/* Footer Modal */}
          <div className="px-6 md:px-16 py-8 md:py-12 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
             <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                INTERVENTION CERTIFIÉE ESEND
             </div>
             <button onClick={onClose} className="flex items-center gap-2 text-white hover:text-red-600 transition-colors">
