...
const Hero = () => {
  ...
  return (
    <section id="accueil" className="relative min-h-screen flex flex-col justify-center overflow-hidden bg-[#020617] text-white pt-20 lg:pt-0">
      {/* Background Layer - Deep Slate / Black */}
      ...
      <div className="max-w-[1440px] mx-auto px-6 w-full flex flex-col lg:flex-row items-center justify-between relative z-10">
        ...
        {/* Left Content - Precise Control */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
          className="lg:w-[45%] flex flex-col justify-center text-center lg:text-left mb-6 lg:mb-0 pt-10 lg:pt-0"
        >
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black tracking-tighter leading-[0.95] uppercase mb-6">
            VOTRE EXPERT <br/>
          ...
          <div className="flex flex-col sm:flex-row gap-6 items-center justify-center lg:justify-start mb-10">
            <motion.a 
              href="#devis"
              onClick={scrollToDevis}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="w-full sm:w-auto bg-red-600 text-white px-10 py-4.5 rounded-2xl font-black uppercase tracking-widest transition-all text-xs shadow-[0_15px_40px_rgba(220,38,38,0.3)] block"
            >
              Obtenir mon devis
            </motion.a>
        ...
        >
          <div className="relative w-full max-w-[550px] lg:max-w-[750px] flex items-end justify-center">
            ...
            {/* Floating Intelligence Card - Mobile: Far Bottom Right / Desktop: Mid Left */}
            <motion.div 
              animate={{ y: [0, -10, 0] }}
              transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
              className="absolute bottom-4 right-0 lg:bottom-[20%] lg:-left-12 bg-slate-900/95 backdrop-blur-xl p-3 lg:p-6 rounded-2xl lg:rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] z-20 border border-white/5 flex items-center gap-3 lg:gap-5"
            >
              <div className="w-8 h-8 lg:w-14 lg:h-14 bg-red-600 rounded-full flex items-center justify-center text-white shadow-[0_0_20px_rgba(220,38,38,0.4)]">
              ...
              <div className="flex flex-col">
                <span className="font-black text-[10px] lg:text-base uppercase tracking-tighter text-white leading-none">PROTECTION TOTALE</span>
                <span className="text-[7px] lg:text-[10px] font-bold text-red-500 uppercase tracking-widest mt-1">GARANTIE RÉSULTAT</span>
              </div>
            </motion.div>
          </div>
          
          {/* Enhanced Glow Effect */}
          <div className="absolute inset-0 bg-red-600/10 rounded-full -z-10 blur-[150px] opacity-20 transform scale-150" />
        </motion.div>
